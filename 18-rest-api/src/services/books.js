...
const applyPagination = (items, top, skip) => {
  ...
  const safeTop = Number.isInteger(parsedTop) && parsedTop > 0 ? Math.min(parsedTop, items.length) : DEFAULT_TOP;
  return items.slice(safeSkip, safeSkip + safeTop);
};

function queryBooks({ filter, select, orderby, top, skip }) {
  const filtered = applyFilter(books, filter);
  const ordered = applyOrder(filtered, orderby);
  const paged = applyPagination(ordered, top, skip);
...
module.exports = { queryBooks, books };
